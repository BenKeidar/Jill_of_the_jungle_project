...
void Board::define_m_board(vector<string>m_file_vector) 
{
	...
	for (int y = ZERO; y < m_length; y++)
	{
		for (int x = ZERO; x < m_width; x++)
		{
			...
			{
				//-------------------------------\\static object//-----------------------------------------
				case floor_object:m_board[y][x] = make_unique <Floor>(location, '#'); //floor
					break;
				case apple_object:m_board[y][x] = make_unique <Apple>(location, 'A'); //Apple
					break;
				case wall_object:m_board[y][x] = make_unique <Wall>(location, 'W'); //Wall
					break;
				case key_gate_object:m_board[y][x] = make_unique <KeyGate>(location, 'G'); //KeyGate
					m_KeyGate.x = x;
					m_KeyGate.y = y;
					break;
				case key_object:m_board[y][x] = make_unique <Key>(location, 'K'); //Key
					break;
				case spikes_object:m_board[y][x] = make_unique <Spikes>(location, 'S'); //Spikes
					break;
				case ladder_object:m_board[y][x] = make_unique <Ladder>(location, 'V'); //Vine
					break;
				case diamond_object:m_board[y][x] = make_unique <Diamond>(location, 'D');//Diamond
					break;
				case gem_gate_object:m_board[y][x] = make_unique <GemGate>(location, 'J'); //gemgate
					break;
				case special_wall_object:m_board[y][x] = make_unique <SpecialWall>(location, '@'); //SpecialWall
					break;
				//-------------------------------\\dynamic object//-----------------------------------------
				case player_object:
					playerLoc = location;
					break;
				case alligator_object:
...
bool Board::isFloating(Player& p) const
{
	...
	if (typeid(*m_board[(int)(pos.y) + ONE][(int)pos.x]) == typeid(Floor) ||
		...
		typeid(*m_board[(int)(pos.y) + ONE][(int)pos.x]) == typeid(Ladder)  ||
...
void Board::set_gift(int x, int y, char icon)
{
	...
	switch (icon)
	{
	case Life: m_board[y][x] = make_unique <PresentLife>(location, '1');
		break;
	case Score: m_board[y][x] = make_unique <PresentScore>(location, '2');
		break;
	case Time: m_board[y][x] = make_unique <PresentTime>(location, '3');
		break;
	case Shield: m_board[y][x] = make_unique <PresentShield>(location, '4');
		break;
	}
}
